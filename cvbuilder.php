...
if (empty($_SESSION)) {
    ...
    <div class="wrapping">
        <div class="container">
            <h2>Construisez votre CV</h2>
            <div class="form-group">
                <form name="add_formation" id="add_formation" method="post">
                    <div class="table-responsive">
                        <table class="table table-bordered" id="dynamic_field">
                            <tr>
                                <td><h3>Vos Formations</h3></td>
                            </tr>
                            <tr id="row'+i+'">
                                <td><input type="text" name="add_formation[]" placeholder="Intitulé du diplôme"
                                           class="form-control name_list"/></td>
                                <?php echo $form->error('add_formation[]'); ?>
                                <td><input type="text" name="add_diplomedate[]" placeholder="Date d'obtention"
                                           class="form-control name_list"/></td>
                                <?php echo $form->error('add_diplomedate[]') ?>
                                <td><input type="text" name="add_lieu[]" placeholder="Etablissement"
                                           class="form-control name_list"/></td>
                                <?php echo $form->error('add_lieu[]') ?>
                                <td><input type="text" name="add_work[]"
                                           placeholder="Tâches effectuées lors de votre formation"
                                           class="form-control name_list"/></td>
                        ...
                        <table class="table table-bordered" id="dynamic_field_xp">
                            ...
                            <tr id="row'+i+'">
                                <td><input type="text" name="add_post[]" placeholder="Intitulé du poste"
                                           class="form-control name_list"/></td>
                                <td><input type="text" name="add_entreprise[]" placeholder="Entreprise"
                                           class="form-control name_list"/></td>
                                <td><input type="text" name="add_place[]" placeholder="Lieu"
                                           class="form-control name_list"/></td>
                                <td><input type="text" name="add_taches[]"
                                           placeholder="Description des tâches effectués"
                                           class="form-control name_list"/></td>
                                <td><input type="text" name="add_time[]" placeholder="Période"
                                           class="form-control name_list"/></td>
                                <td>
                                    <button type="button" name="add_xp" id="add_xp" class="btn btn-success">Add More
                                    </button>
                                </td>
                            </tr>
                        </table>
